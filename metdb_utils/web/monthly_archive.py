def monthly_archive(symLink, baseDir, page):
    ...
    The actual page will be created in a file named <Mon>.html in subdirectory
    <yyyy> under the baseDir argument, which is created if necessary.
    ...
    now = datetime.now()
    outDir = baseDir + now.strftime("/%Y")

    if not os.path.exists(outDir):
        os.makedirs(outDir)

    outFile = outDir + now.strftime("/%b") + ".html"
