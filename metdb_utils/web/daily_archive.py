def daily_archive(symLink,baseDir,page):
  """Write a HTML page to a location based on the current date and create a symbolic link to it.

  This will usually be used for scripts which run on a regular basis, creating
  output that needs to be stored in an "archive" directory, with a link
  to the "current" page.
  The actual page will be created in a file named <dd>.html in subdirectories 
  <yyyy>/<mm> under the baseDir argument - these will be created if necessary.
  The symbolic link supplied in the symlink argument will then be destroyed and 
  re-created, linking to the file written.
