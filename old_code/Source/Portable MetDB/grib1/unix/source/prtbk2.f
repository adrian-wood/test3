C $Header: prtbk2.f, 1, 27/02/2007 16:11:09, Stan Kellett$

      SUBROUTINE PRTBK2 (IB0PAR,IB2PAR,RPAR)
...
C*    EMMA HIBLING   MOD JUNE 1991  POLAR STER/MERCATOR.
C*    EMMA HIBLING   MOD JULY 1991  EDITION 1.
C*    EMMA HIBLING   MOD MAY  1992  ROTATED LAT/LONG.
C*
C**************************************************************
!$Log:
! 1    Met_DB_Project 1.0         27/02/2007 16:11:09    Stan Kellett    
!$
!Revision 1.10  2004/05/20 11:38:47  usmdb
!19 May 2004. Add ECMWF Copyright notice.
!
!Revision 1.9  1999/04/16 09:31:00  usmdb
!Add Lambert conformal projection
!
!$Revision: 1$
!$Date: 27/02/2007 16:11:09$
!
*
************************************************************************
...
*
      ...
      DIMENSION IB2PAR(*),IB0PAR(*)

C     If edition 1, call PRBKN2.

      IF (IB0PAR(1).EQ.1) THEN
        CALL PRBKN2(IB2PAR,RPAR)
        RETURN
      ENDIF
...
C     Gaussian, latitude / longitude and mercator.
     ...
     +    OR.IB2PAR(3).EQ.10) THEN
        ...
        WRITE (*,9102) IB2PAR(4)
 9102   FORMAT (' NO. OF POINTS ALONG A PARALLEL.      ',I9)
        WRITE (*,9103) IB2PAR(5)
 9103   FORMAT (' NO. OF POINTS ALONG A MERIDIAN.      ',I9)
        WRITE (*,9104) IB2PAR(6)
 9104   FORMAT (' LATITUDE OF ORIGIN (SOUTH -IVE).     ',I9)
        WRITE (*,9105) IB2PAR(7)
 9105   FORMAT (' LONGITUDE OF ORIGIN (WEST -IVE).     ',I9)
        NBIT = 8
 ...
 9110     FORMAT(' INCREMENT ALONG A MERIDIAN.          ',
     +           I9)
        ENDIF
 ...
 8110     FORMAT(' NO. OF LATITUDE LINES POLE / EQUATOR.',
     +           I9)
        ENDIF
