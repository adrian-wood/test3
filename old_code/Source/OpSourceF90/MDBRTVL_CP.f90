...
!
!  The data type and number of elements must be in the format (A8,I4).
!
...
! REVISION INFO :
!
! $Workfile: MDBRTVL_CP.f90$ $Folder: OpSourceF90$
...
!-----------------------------------------------------------------------
!     COMMON BLOCK TO ACCESS VARIABLE NAMES FOR PRINTOUT (ALSO IN 'MDB')
!-----------------------------------------------------------------------

CHARACTER*36 USRELM(12000)  ! Element names
CHARACTER*36 ANAME(500)     ! Not used
CHARACTER*9  CIDENT(50)     ! Not used
CHARACTER*4  CICAOL(50)     ! Not used
INTEGER      SELECT(50)     ! Not used  !3
...
!                                                    Read NAMELIST data
 ...
 READ(RECORD,NML=INPUT,IOSTAT=ISTAT5)
!                                                       Stop if problem
  IF (ISTAT5.GT.0) THEN
...
IF (AGAIN) THEN
!                     Read data type and number of retrieved parameters

  READ (RECORD,'(A8,I4)') DATYPE, ITEMS
  WRITE (6,'(//T2,2A/)') 'DATA TYPE ', DATYPE
  READ (5,'(A)') RECORD
...
!                                                        Set up formats
      I = 12*MIN0(IPRINT,NCOLS) + 11
      WRITE (FORMATC,'(A,I3.3,A)')&
          &'(T', I, ',A,T2,I4,2X,10(2X,A10))'
      WRITE (FORMATI,'(A,I3.3,A)')&
          &'(T', I, ',A,T2,I4,2X,1P,10E12.4)'
...
!                                                 Print column headings
        NFIRST = J1 + KOUNTOBS
        NLAST  = J2 + KOUNTOBS
        WRITE (6,'(/T4,10I12)') (J,J=NFIRST,NLAST)
...
!                                                    (1) Character data
          IF (CHARS) THEN
            ...
            WRITE (6,FORMATC)&
              &USRELM(ITEM), ITEM, (CSTR(JOB-J1+J)&
              &(MOD(NINT(RARRAY(J)),65536):&
              &(MOD(NINT(RARRAY(J)),65536)+ILEN-1)), J=J1,J2)
          ELSE
!                                                       (2) Real values
            WRITE (6,FORMATI)&
                    &USRELM(ITEM), ITEM, (RARRAY(J),J=J1,J2)
          END IF
