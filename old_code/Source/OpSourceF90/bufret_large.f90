...
!#######################################################################
!OLD $Revision: 1$
!OLD $Date: 12/02/2010 14:41:52$
!OLD $Source: /home/us0400/mdb/op/lib/source/RCS/bufret.f,v $
! $Workfile: bufret_large.f90$ $Folder: OpSourceF90$
...
!#######################################################################
!OLD  PARAMETER    (MDATA  = 180000)
!OLD  PARAMETER    (MDES   = 4500)                                 !2.13
      PARAMETER    (MDATA  = 300000)
      PARAMETER    (MDES   = 50000)
!#######################################################################
...
!#######################################################################
!OLD  INTEGER I             ! Integer variable for local use
...
!#######################################################################
!OLD  CHARACTER*(MAXBLK) DATBLK     ! Data block read from storage d.s.
CHARACTER*(MAXBLK) DATBLK(MAXDAT) ! Data read from storage d.s.
CHARACTER*(IDHED)  DATHED     ! Data record header
!#######################################################################
CHARACTER*8        ELIST      ! Element index member name     !2.9
!#######################################################################
!OLD  CHARACTER*132      HEAD       ! For revision information
CHARACTER*80       HEAD       ! For revision information
!#######################################################################
...
!#######################################################################
!OLD    HEAD='$RCSfile: bufret.f,v $ ' //
        HEAD = '$Workfile: bufret_large.f90$ ' //&
!#######################################################################
     &'$Revision: 1$ $Date: 12/02/2010 14:41:52$'
!
...
!-----------------------------------------------------------------------
          ...
          IF (IVER.EQ.2 .AND. ICHAR2(NDXBLK(20:21)).EQ.1)THEN !2.1
...
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& DIAGNOSTICS &&&&
WRITE (6,'(T4,A,I3,A,I3,A)')&
     &'Index entry', JXE, ' with', IOVFL, ' overflows'
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
!#######################################################################
...
!#######################################################################
            IF (IBLOCK.NE.NUMDAT) THEN
              READ (IFT,REC=IBLOCK) DATBLK(1)(1:IRLEN)
              NUMDAT = IBLOCK
...
!#######################################################################
          ELSE
            ...
            IF (I.GT.MAXBLK*MAXDAT) THEN
              WRITE (6,'(T2,2A,I3)') 'BUFRET: Message skipped ',&
      ...
      READ (IFT,REC=IBLOCK) DATBLK(1)(1:IRLEN)
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& DIAGNOSTICS &&&&
I = IBLOCK  ! For write statement below
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
            IBLOCK = ICHAR3(DATBLK(1)(MSTART:MSTART+3))
            MSTART = MSTART + 3
            MLEN = MLEN - 3
            NUMDAT = 0
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& DIAGNOSTICS &&&&
WRITE (6,'(T6,A,I5,A,3I7)') 'Initial:    Record', I,&
   &',  MSTART, MLEN, MEND =', MSTART, MLEN, MEND
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&

            I = IRLEN - IDHED
            DO J=1,IOVFL
              READ (IFT,REC=IBLOCK) DATHED,&
                   &DATBLK(1)(MEND+1:MEND+I)
              MLEN = MLEN + I
              MEND = MEND + I
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& DIAGNOSTICS &&&&
WRITE (6,'(T6,A,I2,A,I5,A,3I7)') 'Overflow',J, ', Record',IBLOCK,&
    &',  MSTART, MLEN, MEND =', MSTART, MLEN, MEND
!&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
...
!#######################################################################
!OLD            BufrEdition = ICHAR(DATBLK(MSTART+7:MSTART+7))
                BufrEdition = ICHAR(DATBLK(1)(MSTART+7:MSTART+7))
!#######################################################################
!
...
!
!#######################################################################
!OLD            I3 = I1 + ICHAR3(DATBLK(I1+1:I1+3))   ! Skip section 1
!OLD            I = ICHAR(DATBLK(I1+8:I1+8))          ! Section 1 flags
          I3 = I1 + ICHAR3(DATBLK(1)(I1+1:I1+3)) ! Skip section 1
          I = ICHAR(DATBLK(1)(I1+8:I1+8))       ! Section 1 flags
!#######################################################################
                IF (I.GE.128) THEN                    ! Section 2 exists
!#######################################################################
!OLD              I3 = I3 + ICHAR3(DATBLK(I3+1:I3+3)) ! so skip that too
            I3 = I3 + ICHAR3(DATBLK(1)(I3+1:I3+3)) ! so skip it
          END IF
!#######################################################################
!                                                              Printout
...
!                                                       Year of receipt
          IF (BufrEdition.LT.4) THEN                           !2
            I = I1 + 13                                        !2
!#######################################################################
!OLD              IRCVD(1) = ICHAR(DATBLK(I:I)) ! Year of century    !2
            IRCVD(1) = ICHAR(DATBLK(1)(I:I)) ! Year of century !2
!#######################################################################
...
!                                              Convert year to 4-digits
          ...
          ELSE                                                 !2
!#######################################################################
!OLD              IRCVD(1) = ICHAR2(DATBLK(I1+16:I1+17)) ! Year      !2
            IRCVD(1) = ICHAR2(DATBLK(1)(I1+16:I1+17)) ! Year   !2
!#######################################################################
      I = I1 + 17                                        !2
    END IF                                               !2
...
!#######################################################################
!OLD            IRCVD(2) = ICHAR(DATBLK(I+1:I+1)) ! T.O.R. month     !2
!OLD            IRCVD(3) = ICHAR(DATBLK(I+2:I+2)) ! T.O.R. day       !2
!OLD            IRCVD(4) = ICHAR(DATBLK(I+3:I+3)) ! T.O.R. hour      !2
!OLD            IRCVD(5) = ICHAR(DATBLK(I+4:I+4)) ! T.O.R. minute    !2
          IRCVD(2) = ICHAR(DATBLK(1)(I+1:I+1)) ! T.O.R. month  !2
          IRCVD(3) = ICHAR(DATBLK(1)(I+2:I+2)) ! T.O.R. day    !2
          IRCVD(4) = ICHAR(DATBLK(1)(I+3:I+3)) ! T.O.R. hour   !2
          IRCVD(5) = ICHAR(DATBLK(1)(I+4:I+4)) ! T.O.R. minute !2
!#######################################################################
...
!#######################################################################
!OLD              IRCVD(7) = ICHAR(DATBLK(I1+5:I1+5))  ! Subcentre   !5
            IRCVD(7) = ICHAR(DATBLK(1)(I1+5:I1+5)) ! Subcentre !5
!#######################################################################
          ELSE                                                 !5
!#######################################################################
!OLD              IRCVD(7) = ICHAR2(DATBLK(I1+7:I1+8)) ! Subcentre   !5
            IRCVD(7) = ICHAR2(DATBLK(1)(I1+7:I1+8)) ! Subcentre !5
!#######################################################################
          END IF                                               !5
...
!#######################################################################
!OLD            ILOCD = ICHAR2(DATBLK(I3+8:I3+9))
          ILOCD = ICHAR2(DATBLK(1)(I3+8:I3+9))
!#######################################################################
...
!#######################################################################
!OLD &                        LFLAG, LOCD, DATBLK(MSTART:MEND), NELREQ,
            &LFLAG, LOCD, DATBLK(1)(MSTART:MEND), NELREQ,&
!#######################################################################
...
!#######################################################################
!OLD              LASTOB1 = MLEN  ! To skip message                 !2.5
...
!#######################################################################
!OLD &                     DATBLK(LASTOB1+1:LASTOB1+ICOPY)          !2.3
              &DATBLK(1)(LASTOB1+1:LASTOB1+ICOPY)                   !2.3
!#######################################################################
!
