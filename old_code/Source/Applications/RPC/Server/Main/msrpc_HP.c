/*--------------------------------------------------------------------*
 * Program        : msrpc_HP.c - HP version. Subroutine name = call_mdbp_1
 *
 * Language       : C
 *                : NOTE!! - Some RPC routines included in <rpc/rpc.h>
 *                : don't conform to ANSI C.
 *
 * Description    : This is the RPC C MDB server program on the HP.
 *                : It receives arguments from the RPC C server stub
 *                :( produced using ONC RPCGEN on the HP) and passes
 *                : them to the MetDB Fortran subroutine MDB.
 *
 * Called by      : RPC C server stub.
 *
 * Calls          : MDB     : FORTRAN subroutine MDB
 *
 * Revision info  :
 *
 *
 ...
 * $Source: /home/us0400/mdb/op/lib/RPC/server/main/RCS/msrpc_HP.c,v $
 *
 * Change history :
 *
 * $Log:
 ...
 * $
 * Revision 1.1  2001/01/09  14:14:56  14:14:56  usmdb (Generic MDB account)
 * Initial revision
 * 
 *
 * 04-01-2001   Adapted from IBM version msrpc_IBM.c v1.4 - S.Cox
 *--------------------------------------------------------------------*/
 ...
 * stdlib.h     : needed for malloc calls.
 ...
 * ==================
 *
 * We now call the MetDB. Parameters need to be passed by reference,
 * so that changes may be made to them, so arguments passed must be 
 * addresses. The last 4 arguments are lengths of the character
 * arguments. These are not passed by reference.
 *--------------------------------------------------------------------*/

  timtxt();
  (void)fprintf(stdout,"call_mdbp_1: About to call mdb\n");

  if (request_binary > 0 ) {
    mdb(input_arg->csubt, input_arg->creq,
  ...
  timtxt();
  (void)fprintf(stdout,"call_mdbp_1: Back from mdb\n");
 ...
 * The problem came to light with Cray T3E -> IBM communication.
 *--------------------------------------------------------------------*/
 ...
 * Added 2 more diagnostics around svc_sendreply - S.Cox
 * 
 *
