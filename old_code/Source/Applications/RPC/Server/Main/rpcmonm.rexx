 ...
 *             : multi-user RPC server MDBOA and MDBOB.
 *             : MDBOA and MDBOB are
 ...
 * $
 * Revision 1.7  2003/06/03 09:20:38  usmdb
 * Added check for monitoring job running on wrong system - S.Cox
 *
 * Revision 1.6  2002/07/12 14:32:16  usmdb
 * Check for 'ready and waiting' in all lines of SYSPRINT output - S.Cox
 *
 * Revision 1.5  2001/09/17 11:46:09  usmdb
 * Name changed to RPCMONM. Now works for MDBOA, MDBOB servers
 * only - S.Cox
 *
 ...
 * Initial revision
 *
 * $Source: /home/us0400/mdb/op/lib/RPC/server/main/RCS/rpcmonm.rexx,v $
 * $Revision: 1$
...
if system /= PROD then do
  do forever
    ...
    call CONSMSG "MDBOM01 For info: MDBOM running on wrong system"
    sleep 600
  end
end
...
do loop = 0 to rpcinfo_retries
  ...
  if check_server_rc = 1 then do                               /* 1.6 */
    leave                                                      /* 1.6 */
  end                                                          /* 1.6 */
end
