...
INTEGER, PARAMETER :: LRECL=250000   !- Amount of data to read each time
...
CHARACTER(LEN=*) :: BUFMES  !- BUFR message to return.
CHARACTER(LEN=4) :: BUFR        !- "BUFR"
CHARACTER(LEN=LRECL*2) ::  CS          !- String for two
                                !- consecutive blocks
CHARACTER(LEN=*) :: FNAME       !- Filename to open.
CHARACTER(LEN=132) :: HEAD        !- Revision information
CHARACTER(LEN=4) ::  SEVENS      !- "7777"
...
!-----------------------------------------------------------------------
  ...
  IF (ILEN == LRECL) THEN
    ...
    PRINT*,'CREAD record CS(',LRECL+1,':',2*LRECL,')'
  ENDIF
...
IF (IPOS <= 0) THEN    !- end of data if no 'BUFR' found.
  RC=-1
  CALL METDB_CCLOSE(1)
  FILEOPEN=.FALSE.
  RETURN
...
IF (CS(N+L-4:N+L-1) /= SEVENS) THEN  !- end of data if no '7777'
  RC=3
  CALL METDB_CCLOSE(1)
  FILEOPEN=.FALSE.
  RETURN
