...
!
! $Revision: 1$
! $Date: 30/01/2006 20:21:16$
! $Source: /home/us0400/mdb/op/lib/source/RCS/bufdrpl_sw.f,v $
!
! CHANGE RECORD : 
! $Log:
!  1    Met_DB_Project 1.0         30/01/2006 20:21:16    Sheila Needham  
! $
! Revision 1.2  2005/03/07 13:43:30  usmdb
! added log.
! Stan Kellett. 7th March 2005.
!
!
!-----------------------------------------------------------------------
...
! one follows immediately).
      ...
      IF (.NOT.HEADSET) THEN
        HEAD='$RCSfile: bufdrpl_sw.f,v $ ' //
     &       '$Revision: 1$ $Date: 30/01/2006 20:21:16$'
        HEADSET=.TRUE.
      ENDIF
...
! pointer back so that the increments will be overwritten.
      ...
      ELSE IF (NTIMES.EQ.0) THEN
        IVAL=IVAL-NINCREM           ! set values pointer back
        DESCR(N)=16384+NTIMES+FLDEL ! F=1, NTIMES in XXYYY, flag
                                    ! flag descriptors after operator
        DO I=N+1,N+IMBED+NBUNCH
          IF (MOD(DESCR(I)/FLDEL,2).NE.1) DESCR(I)=DESCR(I)+FLDEL
        ENDDO
                                    ! flag increments before operator
        I=N-1
        DO WHILE (NINCREM.GT.0)
          IF (DESCR(I).LT.16384 .AND. MOD(DESCR(I)/FLDEL,2).NE.1) THEN
            DESCR(I)=DESCR(I)+FLDEL
            NINCREM=NINCREM-1
            I=I-1
          ENDIF
        ENDDO
                                    ! & any operator before increments
! This is the only difference to BUFRDPL.f ver 2.0 AND is added to the  DO
        DO WHILE (DESCR(I).GE.32768 .AND. DESCR(I).LT.FLDEL)
          DESCR(I)=DESCR(I)+FLDEL
          I=I-1
        ENDDO
