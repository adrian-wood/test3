...
!
! $Revision: 6$
! $Date: 22/06/2011 16:30:22$
! $Workfile: cartsub.f$
!
! CHANGE RECORD :
! $Log:
!  6    MetDB_Refresh 1.5         22/06/2011 16:30:22    Richard Weedon  final
!       correction
!  5    MetDB_Refresh 1.4         22/06/2011 14:24:12    Richard Weedon
!       corrected
!  4    MetDB_Refresh 1.3         21/06/2011 14:33:54    Richard Weedon  Update
!        to cartsub for the storage of zfs data set MSGRADFD
!  3    MetDB_Refresh 1.2         17/06/2011 13:13:58    Richard Weedon
!       Updated for the inclusion of MSGRADFD
!  2    MetDB_Refresh 1.1         20/05/2011 13:13:51    Rosemary Lavery Update
!        for ZFS data sets
...
! Local Parameters                                                  !18
      ...
      INTEGER MAXJ                       ! Max num of lines of JCL  !18
      PARAMETER (MAXJ=2000)                                         !18
      CHARACTER*1 QUOTE                  ! Symbol - single quote    !18
      PARAMETER (QUOTE='''')                                        !18
...
! Local Scalars                                                     !18
      ...
      INTEGER IZ,L             !Loop counters                      !18
      INTEGER LC               !line count                         !18
      ...
      INTEGER LENGTH                     !Function to find length   !18
      INTEGER N1
      INTEGER N2
      INTEGER N3
...
!Declare character
      ...
      CHARACTER*80  JCL_LINE(MAXJ)         !JCL to submit           !18
      CHARACTER*80  JCL_COPY(MAXJ)         !JCL to submit (temp)    !18
      ...
      CHARACTER*28  DSN_IN                 !data set to be copied   !18
      CHARACTER*8   DATYPE                 !data subtype            !18
...
!Initialize variables
     ...
     &'$Revision: 6$ $Date: 22/06/2011 16:30:22$'
...
! Re-set flag array to pass all lines of JCL for submission         !18
            DO IZ=1,MAXJ                                            !18
              OJCL(IZ)=.TRUE.                                       !18
            ENDDO                                                   !18
...
! Determine type of data set to be copied and save name and position !18

            DATYPE=LOOKUP_RECORD(1:7)                                !18
!          WRITE (6,'(A,A)') 'DATYPE ',DATYPE
            DSN_IN=LOOKUP_RECORD(36:62)                             !18
! ZFS data set - add name and reset flag to omit MVS lines          !18
! Pathname format is .../subtype/filename                           !18
            IF(INDEX(DSN_IN,'/').GT.0) THEN                         !18
              PATH=DATYPE(1:7)
!             WRITE (6,'(A)') 'PATH SET'
              IF (DATYPE.EQ.'MSGRDFD') THEN
              PATH='MSGRADFD'
!             WRITE (6,'(A)') 'PATH RESET'
              END IF
              POS=INDEX(JCL_LINE(10),'???')                         !18
              N1=LENGTH(PATH)
              N2=LENGTH(DSN_IN)
              JCL_LINE(10)(POS:)=PATH(1:N1) //
     &          DSN_IN(1:N2) // QUOTE
              OJCL(5)=.FALSE.                                       !18
              OJCL(6)=.FALSE.                                       !18
              OJCL(7)=.FALSE.                                       !18
! MVS data set - add name and reset flag to omit ZFS lines          !18
            ELSE                                                    !18
              POS=INDEX(JCL_LINE(7),'???')                          !18
              JCL_LINE(7)(POS:POS+26)=DSN_IN                        !18
              OJCL(8)=.FALSE.                                       !18
              OJCL(9)=.FALSE.                                       !18
              OJCL(10)=.FALSE.                                      !18
            END IF                                                  !18

            JCL_LINE(12)(16:19)=LOOKUP_RECORD(64:67)  !CYL on DPark !18
            JCL_LINE(12)(48:52)=LOOKUP_RECORD(69:73)  !BLKSIZE      !18
            JCL_LINE(13)(14:43)=LOOKUP_RECORD(75:104) !DSN on Dpark !18

            I=INDEX(JCL_LINE(13),'YYMMDD')                         !18
            IF (I.EQ.0) THEN
              ARCHIVE_THEM=.FALSE.
              PRINT*, 'Corrupt record in Lookup Table for ',
     &                LOOKUP_RECORD(36:62)
              PRINT*, 'Archiving is not possible from this datastore'
            ELSE
              JCL_LINE(13)(I:I+1)=CYEAR_TO_ARCHIVE(3:4)            !18
              WRITE(JCL_LINE(13)(I+2:I+3),'(I2.2)')MONTH_TO_ARCHIVE !18
              WRITE(JCL_LINE(13)(I+4:I+5),'(I2.2)')DAY_TO_ARCHIVE  !18

              WRITE(JCL_LINE(15)(1:4),'(I4.4)')YEAR_TO_ARCHIVE     !18
              WRITE(JCL_LINE(15)(5:6),'(I2.2)')MONTH_TO_ARCHIVE    !18
              WRITE(JCL_LINE(15)(7:8),'(I2.2)')DAY_TO_ARCHIVE      !18
              JCL_LINE(15)(10:13)=START_TIME                       !18
              IF (START_TIME.NE.'0000') THEN
!24 hour slot spans a midnight
                CENDAY=CENDAY+1
                CALL DATE13
     ...
     &             YEAR_TO_ARCHIVE)
              ...
              WRITE(JCL_LINE(15)(16:19),'(I4.4)')YEAR_TO_ARCHIVE    !18
              WRITE(JCL_LINE(15)(20:21),'(I2.2)')MONTH_TO_ARCHIVE   !18
              WRITE(JCL_LINE(15)(22:23),'(I2.2)')DAY_TO_ARCHIVE     !18
              JCL_LINE(15)(25:28)=STOP_TIME                         !18
              JCL_LINE(15)(31:35)=LOOKUP_RECORD(69:73) !BLKSIZE     !18
              JCL_LINE(15)(37:66)=LOOKUP_RECORD(112:141) !Any parms !18

! Copy all lines flagged as required                                !18
              LC=0                                                  !18
              DO L=1,LINES_OF_JCL                                   !18
                IF(OJCL(L)) THEN                                    !18
                  LC=LC+1                                           !18
                  JCL_COPY(LC)=JCL_LINE(L)                          !18
                END IF                                              !18
              END DO                                                !18
              LINES_COPY=LC                                          !18
...
!Diagnostic in case of failure
!             DO I=1,LINES_OUT
!               PRINT*, 'JCL_LINE IS ', JCL_COPY(I)
!             END DO

              CALL SUBMIT(JCL_COPY,LINES_COPY)
...
!Now put values in arrays for future reference
          IF (ARCHIVE_THEM) THEN  !Values for stage three
            READ(LOOKUP_RECORD(143:144),'(I2)') I !Stream number    !2.5
            STREAM_TOTALS(I)=STREAM_TOTALS(I)+1
            DATAPARK_NAME(I,STREAM_TOTALS(I))=JCL_LINE(13)(14:43)   !18
            IN_DATAPARK(I,STREAM_TOTALS(I))=LOOKUP_RECORD(212:212)
