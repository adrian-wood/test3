...
! REVISION INFO:
!
! $Workfile: clrmap.f90$ $Folder: OpSource$
...
CHARACTER(LEN=2)  :: CHARNDX    ! 2-byte pointer of blocks to be freed
...
DO JMAP=MAP1,MAP2
   ...
   IF (JMAP == MAP2) IPT2 = MOD(NBLK2-1,NINBLK) + 1 ! Last map blk
!
!                                       Loop over pointers in map block
   DO JPT=IPT1,IPT2
!                          Reset indicator if for specified index block
!
      IF (MAPBLK(JMAP)(NBYTE:NBYTE+1) == CHARNDX) THEN
         MAPBLK(JMAP)(NBYTE:NBYTE+1) = CHAR00
         IF (NUMAP(JMAP) > 0) NUMAP(JMAP) = -NUMAP(JMAP)
         RESET = .TRUE.
      END IF
