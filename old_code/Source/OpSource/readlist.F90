...
!
! $Revision: 2$
! $Date: 18/11/2010 12:29:49$
! $Source: /home/us0400/mdb/op/lib/source/RCS/readlist.F,v $
!
! CHANGE RECORD :
!
! $Log:
!  2    MetDB_Refresh 1.1         18/11/2010 12:29:49    Sheila Needham
!       FILENAME format
...
IF (LISTNAME(1:1) >= '0' .AND. LISTNAME(1:1) <= '9') THEN
   MEMBER = LISTNAME(2:)
...
#else
OPEN (80, FILE=FILENAME, IOSTAT=ISTAT)
#endif
!                                                            Error trap
IF (ISTAT /= 0) THEN
...
! overwritten by first line to be replicated.                      !2.1
  ...
  IF (ANAME(NAMES)(1:10) == 'REPLICATE*') THEN               !2.1
...
IFLABEL2: &
  IF (NAMES == LREPL+NL-1) THEN                              !2.1
    TAG='_  '                                                !2.1
DOLABEL2: &
    DO I=2,NR                                                !2.1
      IF (I < 10) WRITE (TAG(2:2),'(I1)') I                  !2.1
      IF (I >= 10) WRITE (TAG(2:3),'(I2)') I                 !2.1
DOLABEL3: &
      DO J=1,NL                                              !2.1
        NAMES=NAMES+1                                        !2.1
