...
! REVISION INFO :
!
! $Workfile: mobsto.f$ $Folder: pre_refresh$
! $Revision: 1$ $Date: 22/04/2008 10:51:52$
!
! CHANGE RECORD :
!
! $Log:
!  1    Met_DB_Project 1.0         22/04/2008 10:51:52    Brian Barwell
!       Initial version of storage routine for mobile SYNOP data.
! $
!    MOBSTO was originally created by modifying SYNSTO, March 2008.
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      PARAMETER (MAXBLK=27998,INDLEN=23,INDHED=6,MAXOFL=3)
...
! USE BLOCKX AS A WORK AREA FOR SORTS, EQUIVALENCING THE ARRAYS NEEDED
      ...
      DATA        EMPTYX/' '/
      DATA        MSGHDR/'....Z MOBSTO: '/
      DATA        FIRST /.TRUE./

      IF (FIRST) THEN
        HEAD = '$Workfile: mobsto.f$ ' //
     &         '$Revision: 1$ $Date: 22/04/2008 10:51:52$'
...
! FIND LENGTH OF IDENT FOR USE IN CONSISTENCY CHECK LATER
      ...
      LIDENT=LEN(IDENT)
      DO WHILE (IDENT(LIDENT:LIDENT).EQ.' ')
        LIDENT=LIDENT-1
      ENDDO
...
!  60 is an arbitrary number.)
        ...
        IF(INDEX(BLOCK(DISPL+1:DISPL+60),IDENT(1:LIDENT)).EQ.0)THEN
          PRINT *,MSGHDR,' >>>>>>> DATA BASE INCONSISTENT! <<<<<<<<'
...
! IF STRING OF SAME LENGTH, AND STRING ITSELF IS SAME, THEN DUPLICATE
        ...
        IF (LASTHR.EQ.INDHOR .AND.
     &     (BLOCK(DISPL+L-21:DISPL+L-21).EQ.ENTRY(2:2))) THEN
...
! ENOUGH TO CROSS BLOCKS.)  AT END, RESET POINTER TO START OF CHAIN.
        ...
        NCHAIN=NCHAIN-1
        IF (POINTER(3:4).EQ.INDEKS(IND,LX)(22:23)
     &      .AND. NCHAIN.GT.0) GO TO 320
...
! or has more good values then prefer the new report.
        ...
        IF (BLOCK(DISPL+7:DISPL+7).LT.ENTRY(7:7) .OR.     ! COR FLAG
...
! COUNT OF OBS IS ONLY ONE BYTE; LEAVE IT AS 255 IF MORE THAN 255!
        ...
        IF (IND.GT.0) THEN
          IF (POINTER.LT.BLOCK(START+L-3:START+L)) THEN
...
!                                          ! (IND IS SET IF IDINB=1!)
          ...
          IF (IORC.NE.0) CALL SYSABN (865)

          WRITE (IFT,REC=1,IOSTAT=IORC) MAP
          IF (IORC.NE.0) CALL SYSABN (866)
...
! full block, reduce NFULL to get the range of entries right.
        ...
        IF (NFULL.EQ.0 .OR. NTRIES(LX).EQ.NBLIND) THEN
