...
! REVISION INFO :
!
! $Workfile: tfmret.f$ $Folder: pre_refresh$
! $Revision: 7$ $Date: 25/02/2011 15:35:43$
!
! CHANGE RECORD :
!
! $Log:
!  7    Met_DB_Project 1.6         25/02/2011 15:35:43    Brian Barwell
!       Increase MDATA to 1263 to agree with TAFINT (overlooked in June 2008).
!  6    Met_DB_Project 1.5         01/10/2010 16:33:58    Brian Barwell
!       Additions to retrieve station heights for METARs.
!  5    Met_DB_Project 1.4         12/04/2010 12:51:14    Sheila Needham
!       Increase field size used to hold report length to 5 chars
!  4    Met_DB_Project 1.3         16/12/2008 11:30:44    Brian Barwell
!       Expect map in record 1 even if storage data set has >12000 records.
!  3    Met_DB_Project 1.2         07/08/2008 11:47:34    John Norton
!       Updated with header statements
!  2    Met_DB_Project 1.1         04/08/2008 12:17:43    John Norton
!       Updates required after reviewing change
!  1    Met_DB_Project 1.0         30/01/2006 20:25:24    Sheila Needham  
! $
...
!-----------------------------------------------------------------------
      ...
      INTEGER        IBLOCK       !                                  !i
      INTEGER        ICHL1        !                                  !i
      INTEGER        ICHAR2       ! C*2 to I*4 conversion function   !6
      INTEGER        ICHL2        !                                  !i
      INTEGER        ICHL3        !                                  !i
      INTEGER        ICMIN        !                                  !i
      INTEGER        ID           !                                  !i
...
! possible value that can be held in a 2 byte integer when the sign   !H
...
!-----------------------------------------------------------------------
! Get METAR station height if available (bytes 9-10 of index entry)  !6
!-----------------------------------------------------------------------

          IF (CTYPE(1:6) .NE. 'METARS' .OR.   ! Not METARS or ..     !6
     &        CNTRY(J2)(9:10).EQ.'  ') THEN   ! height missing       !6
            ISECT1(9) = -9999999                                     !6
          ELSE                                                       !6
            I = ICHAR2(CNTRY(J2)(9:10))                              !6
            IF (I.GT.32767) I = I - 65536                            !6
            ISECT1(9) = I                                            !6
          END IF                                                     !6
...
!-----------------------------------------------------------------------
     ...
     &               IDREC,IRECLN,KMSG,'MSGRD')                    !2.1
          ...
          WRITE(CREP(1:5),'(I5)')IRECLN+IHDLEN                    !ST5
     ...
     &                  IY,ISECT1(4),ISECT1(5),NAMD,NCOR,CCCC)    !1.13a
          ...
          CREP(5+1:5+IHDLEN)=HEADER                                !ST5
          CREP(5+IHDLEN+1:5+IHDLEN+IRECLN)=KMSG(1:IRECLN)          !ST5
...
9014              format(' values after tafexp'/ (5F13.2) )          !7
                ENDIF
