...
!-----------------------------------------------------------------------
! Version of WEBRET for test retrieval pages. Uses DDICT call parameter
!-----------------------------------------------------------------------
!
! routine       : WEBRETST
!
...
! REVISION INFO:
!
! $Workfile: webretst.f90$ $Folder: OpSource$
! $Revision: 4$ $Date: 08/03/2013 16:54:43$
!
!-----------------------------------------------------------------------
! change record :
!
! $Log:
!  4    Met_DB_Project 1.3         08/03/2013 16:54:43    Rosemary Lavery
!       ATDNET added
!  3    MetDB_Refresh 1.2         20/07/2012 10:17:02    John Norton     Latest
!        version
!  2    MetDB_Refresh 1.1         11/05/2012 16:09:16    John Norton     Check
...
CHARACTER(LEN=56)   ::  DDICT   !- DDICT keyword for call to MDB
...
! routine belongs to is SYS1.SDBLOAD(WEBRETST).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
! Set DDICT dataset if required.
!-----------------------------------------------------------------------

IF (LEN(TRIM(DDICT)) > 0) THEN
  IF (INDEX(DDICT,'"') > 0) THEN
    DDICT= 'DDICT ' // DDICT // ' '
  ELSE
    DDICT= 'DDICT "' // TRIM(DDICT) // '" '
  END IF
ELSE
  DDICT= 'DDICT "TDB.RETRIEVL.TABLE" '
END IF
...
!-----------------------------------------------------------------------
  ...
  CREQ  = VERSIONSTR // ' ' // PLATFORM // ' ' // &
          ...
          'END TIME '   // ENDDATE // '/' // ENDHOUR // &
                           ENDMINUTE // 'Z ' // &
          INCREMENT  // ' ' // KEY_PARTS // ' ' // AREA // DDICT // &
           ELEMENTS

!#JN      INCREMENT  // ' ' // KEY_PARTS // ' ' // AREA // ' TEST ' // &
!#JN       DDICT // ELEMENTS
!-----------------------------------------------------------------------
...
IFLABEL6: &
      ...
      ELSE
        WRITE(6,*)'WEBRETST: No format routine for subtype ',CSUBT
        TOTALOBS=0
