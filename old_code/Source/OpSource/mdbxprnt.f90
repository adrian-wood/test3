...
! REVISION INFO :
!
! $Workfile: mdbxprnt.f90$ $Folder: OpSource$
! $Revision: 1$ $Date: 21/11/2011 09:39:09$
!
! CHANGE RECORD :
!
! $Log:
!  1    Met_DB_Project 1.0         21/11/2011 09:39:09    Sheila Needham  Moved
!        from UTILITY.SRCE and updated for ZFS datasets (must now be compiled
!       with F95).
! $
...
!     LENREC=27998
      OPEN (10,FILE='DD:INPUT',ACTION='READ')
      READ (10,NML=INPUT)
      CLOSE(10)
!                             Re-open storage data set as direct access

      OPEN (1, FILE='DD:MDBSTORE', ACCESS='DIRECT', RECL=LENREC, &
            ACTION='READ')
!                                                    Read header record
