...
! REVISION INFO :
!
! $Workfile: msgsubn.f90$ $Folder: OpSource$
! $Revision: 3$ $Date: 18/11/2010 10:36:58$
!
! CHANGE RECORD :
!
! $Log:
!  3    MetDB_Refresh 1.2         18/11/2010 10:36:58    Sheila Needham
!       Removed HEAD statements
!  2    MetDB_Refresh 1.1         10/11/2010 15:53:28    Richard Weedon  Peer
!       review - rework completed
!  1    MetDB_Refresh 1.0         28/10/2010 16:30:31    Richard Weedon
...
INTEGER,INTENT(IN)           ::    VALDIM
INTEGER,INTENT(IN)           ::    MEXT        ! Size of IEXTRA array
INTEGER,PARAMETER            ::    SEGDIM=1900 ! Dim of exp seg Table
...
INTEGER,INTENT(IN)  ::   INPUT_SEGNUM(INPUT_NSEG)  ! Segment number
INTEGER,INTENT(IN)  ::   INPUT_STYP(INPUT_NSEG)!Seg type (8=replication)
INTEGER,INTENT(IN)  ::   INPUT_NDSLEN(INPUT_NSEG)! No. of desc in seg
INTEGER,INTENT(IN)  ::   INPUT_NVALEN(INPUT_NSEG)!No. of vals in seg
...
REAL,INTENT(OUT)             ::  VALUES(VALDIM)!VALUE ARRAY FOR DECODE
CHARACTER(LEN=*),INTENT(OUT) ::  CNAME ! char strings for decode
...
! original message used delayed replication.
  ...
  IF (STYP(I) == 0) THEN  ! Not a replication segment
    NREPL(I) = 1
  ELSE
...
! Check for replicated segment.
  ...
  IF_CONSTR1 : &
  IF (NESTED .AND. STYP(I) == 8) THEN  ! Replication           
...
! segment '21'). 'NSUBSEG' includes main segment and all sub-segments.
      ...
      NSUBSEG = 1
      DO WHILE (I+NSUBSEG <= NSEG .AND.&
          SEGNUM(I+NSUBSEG) > SEGNUM(I)*10)
        NSUBSEG = NSUBSEG + 1
      END DO

! If the replication count is zero, delete all the sub-segments from  
! the segment table arrays and update the total number of segments.   
      ...
      IF (NREPL(I) == 0) THEN                                   
        ...
        IF (NSUBSEG > 1) THEN                                   
          NEXTRA = NSUBSEG - 1    ! No. of segments to delete   
          NSEG = NSEG - NEXTRA    ! Update number of segments   
          DO J=I+1,NSEG                                         
            SEGNUM(J) = SEGNUM(J+NEXTRA)                        
            STYP(J)   = STYP(J+NEXTRA)                          
            NDSLEN(J) = NDSLEN(J+NEXTRA)                        
            NVALEN(J) = NVALEN(J+NEXTRA)                        
          END DO                                                
        END IF                                                  
...
! to take them.  Stop if they aren't.
      ...
      ELSE                                                     !2
        ...
        IF (NSEG+NEXTRA > SEGDIM) THEN   ! Arrays not big enough
...
!  "ELMSEG/J > 0" was changed to "ELMSEG/J > 1" below to allow      !1.4
...
! numbered NSTOP is reached.
          ...
          IF_CONSTR9 : &
          IF (ELMSEG > NSEG .OR. SEGNUM(ELMSEG) == NSTOP) THEN
            SEARCH = .FALSE.
...
! STYP of 1 earlier so they can be identified here.
          ...
          ELSE IF (SEGNUM(ELMSEG) == LSEG(L) .AND.&
          (IVAL(I) /= 0 .OR. STYP(ELMSEG) == 1)) THEN
...
! the first replication (STYP=1) whether the count is zero or not.
            ...
            IF_CONSTR10 : &
            IF (L == LEVREQ) THEN
              IF ((IVAL(I) /= 0 .AND. NREPL(ELMSEG) > 0) .OR.&
             (IVAL(I) == 0 .AND. STYP(ELMSEG) == 1)) THEN

! Increment replication count and check against that required.

                NTIMES = NTIMES + 1
                IF (NTIMES == NTH(L)) NOMATCH = .FALSE.
              END IF
...
! replications.
              ...
              IF (NTIMES == NTH(L)) THEN
                IF (LREPL > 0.OR.IVAL(I) == 0) NOMATCH = .FALSE.
...
! to it. In this case add 2**18 to the VALUES subscript indicator too.
    ...
    ELSE
      ...
      IF (NTHS > 1) DISPL(I) = DISPL(I) + (NTHS-1)*NVALEN(ELMSEG)
      IF (MOD(IRDES(IDSC(I)+DSEGST(ELMSEG)-1)/IBIT18,2) == 1)&
