...
! REVISION INFO:
!
! $Workfile: mdbxtrct.f90$ $Folder: OpSource$
! $Revision: 4$ $Date: 14/02/2012 09:18:13$
!
! CHANGE RECORD:
!
! $Log:
!  4    MetDB_Refresh 1.3         14/02/2012 09:18:13    Sheila Needham  Change
!        SYSIN input to FT05 for compatability with MDBARK.
!  3    MetDB_Refresh 1.2         15/06/2011 15:57:31    Brian Barwell   Add
!       fix for index entry bug which occurred 14-15 June 2011.
!  2    MetDB_Refresh 1.1         20/05/2011 12:21:15    Rosemary Lavery corr
!       to declaration
!  1    MetDB_Refresh 1.0         17/05/2011 16:10:31    Rosemary Lavery Update
!        for use with ZFS files
! $
...
!                                                             Variables
      ...
      INTEGER IFT             ! FT unit number for input d/set       !3
      ...
      INTEGER IRC             ! Return code from i/o routines        !3
      ...
      INTEGER JFT             ! FT unit number for output d/s        !3
      ...
      INTEGER MODE            ! Method for open stmt                 !3
      ...
      INTEGER NOUT            ! Record number for output             !3
      ...
      INTEGER NUMR            ! Record number for direct i/o         !3
      ...
      CHARACTER*(MAXREC) DHEAD      ! Header from data record        !2
...
!-----------------------------------------------------------------------
      ...
      IFT=1                                                          !3
      MODE=0                                                         !3
      CALL METDB_COPEN(IFT,'DD:OLD'//CHAR(0),MODE,IRC)               !3

      JFT=3                                                          !3
      MODE=3                                                         !3
      CALL METDB_COPEN(JFT,'DD:NEW'//CHAR(0),MODE,IRC)               !3
...
!                                                    Read header record
      NUMR=1                                                         !3
      CALL METDB_CREAD_DIR (IFT,RECORD,LENREC,NUMR,IRC)              !3

      NVER=TRANSFER(RECORD(1:4),NVER)                                !3
      NRECS1=TRANSFER(RECORD(5:8),NRECS1)                            !3
      NMRECS1=TRANSFER(RECORD(9:12),NMRECS1)                         !3
      NXRECS1=TRANSFER(RECORD(13:16),NXRECS1)                        !3
      NXMIN=TRANSFER(RECORD(17:20),NXMIN)                            !3
      NX00Z=TRANSFER(RECORD(21:24),NX00Z)                            !3
      LENTRY=TRANSFER(RECORD(25:28),LENTRY)                          !3
...
!                                       Number of index periods to copy
      ...
      WRITE (6,'(T3,A,I4,I5,A)') 'Data selected from', NXRECS2,  &
         NXMIN, '-minute index periods.'
...
!                                       Read start of base index record
        ...
        CALL METDB_CREAD_DIR (IFT,RECORD,LENREC,NBASE1,IRC)       !3
!                                                        Check time tag
...
!                                                Read next index record
          CALL METDB_CREAD_DIR (IFT,RECORD,LENREC,NEXTNDX,IRC)       !3
...
!        Fix for index entry bug in storage where data record overflow
!         byte got set to a space rather than a zero (15-16 June 2011)

            IF (NUMOVFL > 60) THEN          ! Overflow byte is a space
              NUMOVFL = 0                   ! But no overflow records
              RECORD(IPT-9:IPT-9) = CHAR(0) ! Reset to zero
            END IF
!                                          Set pointers if not yet done
            IF (NUMNEW(NEXTDAT).EQ.0) THEN                           !2
!                                                      Read data record
              CALL METDB_CREAD_DIR (IFT,DATREC,LENREC,NEXTDAT,IRC)   !3
...
!                                            Read header of data record
                CALL METDB_CREAD_DIR (IFT,DHEAD,LENREC,NEXTDAT,IRC)  !3
!                                            Error message if wrong tag
...
!-----------------------------------------------------------------------
      ...
      RECORD(5:8) = TRANSFER(NRECS2,RECORD(5:8))                     !3
      RECORD(9:12) = TRANSFER(NMRECS2,RECORD(9:12))                  !3
      RECORD(13:16) = TRANSFER(NXRECS2,RECORD(13:16))                !3

      CALL METDB_CWRITE_DIR (JFT,RECORD,LENREC,NUMR,IRC)             !3
...
!                                                     Output map record
        ...
        CALL METDB_CWRITE_DIR (JFT,RECORD,LENREC,NOUT,IRC)           !3
      END DO
...
!                                                       Read the record
          I = IABS(IOLD)
          CALL METDB_CREAD_DIR (IFT,RECORD,LENREC,I,IRC)             !3
...
!                                Close data set, print message and stop
      CALL METDB_CCLOSE (IFT)
      CALL METDB_CCLOSE (JFT)
      WRITE (6,'(T3,A,I6)')  &
