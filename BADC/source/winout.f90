      SUBROUTINE WINOUT(RARRAY,MAXOBS,MAXELS,  &
...
! LOOP THROUGH REPORTS PASSED FROM MDBRET
      ...
      DO OBNUM =1,NOBS                        ! REPORTS LOOP
        ...
        N_LEVELS=MIN(MAX_LEVELS,NINT(RARRAY(OBNUM,29)))
!
!* OUTPUT RECORDS
!
        WRITE(10,  &
        ...
        'WINPRO',N_HEADER_ELEMS,N_LEVELS,NELEMS_PER_LINE,  &
        (DATE(J),J=1,5),(TOR1(J),J=1,5),  &
        (RARRAY(OBNUM,J),J=11,28)

        DO I = 1,N_LEVELS
          K=(I-1)*NELEMS_PER_LINE+REP_START
          L=K+NELEMS_PER_LINE-1
          WRITE(10,'(1X,F13.3,10('','',F13.3))',IOSTAT=IOS,ERR=9999)  &
...
!----------------------------------------------------------------------
 ...
 9990 FORMAT(' ERROR ',I4,' occurred while writing out WINPRO data')
