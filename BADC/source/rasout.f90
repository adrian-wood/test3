      SUBROUTINE RASOUT(RARRAY,MAXOBS,MAXELS,  &
                    NELEM,NOBS,CSTR,IERR)
!-----------------------------------------------------------------------
!
! SUBROUTINE    : RASOUT
!
! PURPOSE       : Output RASS data for BADC
!
...
! LOOP THROUGH REPORTS PASSED FROM MDBRET
      ...
      DO OBNUM =1,NOBS                        ! REPORTS LOOP
        ...
        N_LEVELS=NINT(RARRAY(OBNUM,27))  ! No ore replications
!
!* OUTPUT RECORDS
!
        WRITE(10,  &
             ...
             4('','',I2.2),'','',I4.4,4('','',I2.2),  &
             9('','',F13.3),'','',F13.1,  &
             6('','',F13.3))',IOSTAT=IOS,ERR=9999)  &
        'RASS  ',N_HEADER_ELEMS,N_LEVELS,NELEMS_PER_LINE,  &
        (DATE(J),J=1,5),(TOR1(J),J=1,5),  &
        (RARRAY(OBNUM,J),J=11,26)

        DO I = 1,N_LEVELS
          K=(I-1)*NELEMS_PER_LINE+REP_START
          L=K+NELEMS_PER_LINE-1
          WRITE(10,'(1X,F13.3,5('','',F13.3))',IOSTAT=IOS,ERR=9999)  &
...
!----------------------------------------------------------------------
 ...
 9990 FORMAT(' ERROR ',I4,' occurred while writing out RASS data')
