      SUBROUTINE PLTOUT(RARRAY,MAXOBS,MAXELS,  &
                    NELEM,NOBS,CSTR,IERR)
!-----------------------------------------------------------------------
!
! PROGRAM       : PLTOUT
!
! PURPOSE       : Output PILOT data for BADC
!
...
!-----------------------------------------------------------------------
        ...
        IF(NINT(RARRAY(OBNUM,45)).EQ.0)THEN
          N_LEVELS=NINT(RARRAY(OBNUM,46))
        ELSE
          N_LEVELS=MAX_LEVELS
        ENDIF
!
...
!
        WRITE(10,  &
             ...
             5('','',I8),25('','',I8),'','',A9,  &
             5('','',F13.3),'','',A4,'','',A4,  &
             7('','',F13.3))',IOSTAT=IOS,ERR=9999)  &
        'PILOT ',N_HEADER_ELEMS,N_LEVELS,NELEMS_PER_LINE,  &
        (DATE(J),J=1,5),(TOR1(J),J=1,25),  &
         CALLSIGN,  &
        (RARRAY(OBNUM,J),J=32,36),  &
         CCCC,BULLID,  &
        (RARRAY(OBNUM,J),J=39,45)

        DO I = 1,N_LEVELS
          K=(I-1)*NELEMS_PER_LINE+REP_START
          L=K+NELEMS_PER_LINE-1
          WRITE(10,'(1X,F13.3,4('','',F13.3))',IOSTAT=IOS,ERR=9999)  &
...
!----------------------------------------------------------------------
 ...
 9990 FORMAT(' ERROR ',I4,' occurred while writing out PILOT data')
