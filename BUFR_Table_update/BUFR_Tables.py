...
#
#       Description. 
#    1. Add new Version of Global Table B (requires XML version of Table B)
#    2. Add New Version of Global Table D (requires XML version of Table D)
...
#    5. Compare new and old Table B revisions (requires New Version to be XML)
#    6. Compare new and old Table D Revisions (requires New Version to be XML)
...
#    
#   
#       The above assumes new versions of the BUFR tables and codefigs to have been downloaded from the WMO BUFR Coding pages -
...
#                                      Where entrys from the new xml tables are found in the existing tables a new revision will 
#                                      be added. Where new entries are found without a corresponding entryb in the existing table 
...
#                                       masterg - Dictionary new entries parsed from the WMO xml tables.
...
#                                      Where entrys from the new xml tables are found in the existing tables the new revision will 
...
#   ----------------------------------------------------------------------------------------------------------------------------
#   
#     getDatB(tablenb)                 Will parse and read into a dictionary the contents of the new (xml) version
...
#
#                                      input -   tablenb   - Name of the new BUFR table B (xml format).
...
#   ----------------------------------------------------------------------------------------------------------------------------
#     
#     getDatD(tablend)                 Will parse and read into a dictionary the contents of the new (xml) version
...
#
#                                      input -   tablend   - Name of the new BUFR table D (xml format).
...
#                                      descriptotrs. The data will be read into a dict 
...
#                                                N_ver   - WMO Version of the current operational tables (xml).   
...
#
# purpose:         check for the presence of the tables
#
# input:           
...
#
       if not os.path.isfile(tablexf):
           print "Global BUFR TableB not found at =>  ",tablexf
           exit() 
...
#       
       ...
       if not os.path.isfile(tablexf):
           print "Global BUFR TableD not found at =>  ",spathx
           exit()
       if not os.path.isfile(tableof):
...
#  
       ...
       if not os.path.isfile(tablexf):
           print "Global Common Code Table not found at =>  ",tablexf
           exit()
       if not os.path.isfile(tableof):
...
#  
       ...
       if not os.path.isfile(tablexf):
           print "Global BUFR TableD not found at  =>  ",tablexf
           exit()
       if not os.path.isfile(tableof):
           print "Global BUFR TableB not found at =>   ",tableof
           exit()
       if not os.path.isfile(tableqf):
           print "Global BUFR TableC not found at =>   ",tableqf
           exit() 
   return()
#      
#####################################################################################################################################
...
#
######################################################################################################################################
def final_dictb(masterl,masterg):
  fdict={}
  file_final="Bufr_final.txt"
  fn=open(file_final,'w') #
  for key in masterl.keys():                           # loop through keys (FXY) in the local tables   
     if key in masterg:                                # check for the presence of the key in the WMO version
        listl=[]
	listg=[]
	listtmp=[] 
        chk=0
	sl=len(masterl[key])
        listl=masterl[key][0][1:6]
        listg=masterg[key][:5] 
        listg[0]=listg[0].upper()
        listg[1]=listg[1].upper()
        stlistl=set(listl)
        stlistg=set(listg)
        match=stlistl.difference(stlistg)
        matchl=list(match)
        len_mch=len(matchl)
	if len_mch == 0:                               # if both versions of the desc match 
	    fdict[key] = masterl[key]                  # populate the output dict with the local values 
                                                       # NOTEA TO SELF THIS IS IN ERROR OVERWRITING WITH THE 1ST ELEMENT OF THE LIST CHECK S2 INSERT BELOW
	    fdict[key][0]= masterg[key][0]             # set the revision no for the entry to that in the WMO table 
	else:
	    listtmp=masterl[key]                       # if the versions do not match insert (nest )the local version
	                                               # as a list into a temp list.
	    listtmp.insert(0,(masterg[key]))           # repeat the process for the global version of the same desc.
	    fdict[key]=listtmp            
     else:
        fdict[key]=masterl[key]                        # if key does not exist in the final dict add the local version   
  ...
  for key in masterg.keys():                           # loop through global (WMO) dict and check for their presence in 
     ...
     if key not in fdict:                              # if the key is not in fdict 
        gtemp.insert(0,masterg[key])      
        fdict[key]= gtemp                              # nest the masterg entry in a temp list and add to fdict
  ...
  for key in sorted(fdict.keys()):                     # ascertain which fdict entrys have multiple versions
     ...
     if templ > 1:                                     
        for y in range(0,templ-1):                     # spool through sub sets of the list    
          if y < templ-2:
            list1=temprev[y][3:6]
            list2=temprev[y+1][3:6]
            testm=set(list1) & set(list2)
            if len(testm) < 4:
               match="N"
               break
     if match == "Y":
        fdict[key]=temprev[0:1]
     else:
        for x in range(0,templ-1):
            tiprev=temprev[x][0]
            tiprev="+"+tiprev
            temprev[x][0]=tiprev   
  return fdict  
...
def output_b(f_dict,UKMO_ver,WMO_ver,build_dir):
   ...
   ver_string="   $Revision: "+S_UKMO+"$  $Date: "+xtf+" UTC$                WMO Version "+S_WMO+"\n" 
   for key in sorted(f_dict.keys()):
      y=len(f_dict[key])
      
      if y > n:                     # n - establish the max no of elements in a list
        n=y
   vstring='{message:{fill}{align}{width}}'.format(message=ver_string,fill=' ',align='<',width=72)	
	...
	if x < lenf:
	  ...
	  if (len(string1)) > 60:
	     string1=string1[0:59]                                                                           
          fkey='{message:{fill}{align}{width}}'.format(message=key,fill=' ',align='<',width=8)	                              # F X Y identifier      
          bf.write(('%s')%(fkey)) 
          #   
          string1.upper()                                                                        
          fstring1='{message:{fill}{align}{width}}'.format(message=string1,fill=' ',align='<',width=66)                      # F X Y Description 
          bf.write(('%s')%(fstring1)) 
          #                             
	  string3=f_dict[key][x][0] 
	  string3=string3.strip()  
          fstring3='{message:{fill}{align}{width}}'.format(message=string3,fill=' ',align='<',width=10)	                      # version number  
          bf.write(('%s\n')%(fstring3))
          #
...
def output_d(fdictd,revn,wmo_rev,build_dir):
 ...
 reni=reni+1
 renc=str(reni)
 rev_string="  $Revision: "+renc+"$  $Date: "+xp+"$\n"
 
 fdo=open(fdict_out,'w') #
 fdo.write(('%45s') % (rev_string)) 
 fdo.write(('%45s\n') % (" "))
 #
 for key in sorted(fdictd):
   ...
   desct=fdictd[key][0]                                                                         # sequence title 
   descts=desct.strip(" ")
   if len(descts) == 0:
      desct="NIL"
   desc_c=fdictd[key][1] 
   ...
   fdesct='{message:{fill}{align}{width}}'.format(message=desct,fill=' ',align='<',width=70)    # format output for descriptor title   
   if desct != "NIL" : 
    ...
    for key in nvers.keys():               # where entries are in both dict / tables
      if key in overs:
        ...
        for z in range(0,lenov):
          ...
          tlstb.append(tlsta)
  #      print "tlstb ",tlstb,"\n"
        flist=set(tuple(x) for x in tlstb)             # produces set of non matching tuples
...
#
######################################################################################################################################
def link_common_tables(mfdict2):
#
...
#   print "rep_d : odictn[key] ",key," ",odictn[key],"\n"
   ilistd=[]
   ilistd=odictn[key][2:]
   D_test="True"
...
#  
	...
	(f_dictb)=final_dictb(new_entry,mastergb)   
	output_b(f_dictb,rev_now,wmo_version,hdiri)	        							       							           
# 
    elif inp == "2":                                                       # Update current operational version of Table D from New WMO version 
        Tno="D"
	table_check(inp,hdiri,tablegDx,tableoDt,tablegCx)         
	(mastergd,wmo_version)=getDatd(tablegDx)
...
# 
        ...
        mf_dict=link_common_tables(mfdict)
