...
#
 ...
 echo "==================================================================================================================================="
 echo " " 
 read -p "Y - Create a new version of the build directory , N - Use an existing build directory " response1
 echo " " 
 echo "==================================================================================================================================="
 if [ "$response1" == "$Yes" ]; then                                                                        # N - Take build directory name from the title of the .zip file 
 ...
 if [ "$response1" == "$Yes" ]; then                                                                        # construct new build directory                           
    mkdir $bdir
    mkdir $bufb
    mkdir $bufc
    mkdir $bufd         
    ...
    if [ -e $zfdir ]; then   
         if [ ${zfdir: -4} == ".zip" ]; then                                                                       
             ...
             echo "BUFR gzip file found ( $zfdir ), extracting file listing to $fileB"
             tar tzf $zfdir | grep -e "[^/]$" >> $fileB
         fi   
    else
    ...
    if [ -e $zfdircf ]; then    
         if [ ${zfdircf: -4} == ".zip" ]; then                                                                      
            echo "BUFR Common Code Tables Zip file found ( $zfdircf) , extracting file listing to $fileC"
            zipinfo -1 $zfdircf >> $fileC
         else
             echo "BUFR gzip file found ( $zfdir ), extracting file listing to $fileC"
             tar tzf $zfdircf | grep -e "[^/]$" >> $fileC
         fi
    else
...
if [ "$optin" != "$ten" ]; then
  ...
  if [ "$optin" == "$eight" ]; then 
     ...
     read -p "=> " seq_id 
     echo "sequence ID set $seq_id "
  fi
  if [ "$optin" == "$nine" ]; then 
     refx=/home/developers/richard.weedon/old_code/test_list.txt
     rm $refx
     results=/home/developers/richard.weedon/old_code/elements_index_listing/*.txt
     stat -c %n $results >> $refx
     echo $refx," ",$results  
     echo "Enter the sequence to be expanded." 
     read -p "=> " seq_id 
     echo "sequence set $seq_id "
  fi
else
...
#   $seq_id      =>      Table D ID 
#   $refx        =>      Listing of element_index files (used in the expansi9on of D sequences and creation of elements index outline
#
##############################################################################################################################################################################################
#
  final_arr+=($optin)
  ...
  final_arr+=($seq_id)
  final_arr+=($refx)
  python BUFR_Table_app.py ${final_arr[@]}
